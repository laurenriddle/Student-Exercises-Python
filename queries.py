    ...
    def all_instructor_assignments(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            for instructor_Id, objects in assignments.items():
                ...
                for object in objects["exercises"]:
                    print(f'\t* {object}')
    def all_assignment_students(self):
        """Retrieve all exercises"""
        assignments = dict()
        with sqlite3.connect(self.db_path) as conn:
            db_cursor = conn.cursor()
            ...
            for row in all_Assigments:
                ...
                if exercise_Id not in assignments:
                    assignments[exercise_Id] = {"name": exercise_name,
                    "exercises": {student_name} }
                else: 
                    assignments[exercise_Id]["exercises"].add(student_name)
            
            for exercise_Id, objects in assignments.items():
                print(f'{objects["name"]} is being worked on by:')
                for object in objects["exercises"]:
                    print(f'\t* {object}')
...
reports.all_assignment_students()
